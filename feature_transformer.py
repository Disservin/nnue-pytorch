...
void feature_transformer_slice_forward(
    ...
    __shared__
          half           shared_output[{output_size}];

    const uint32_t       block_idx           = blockIdx.x;
    const uint32_t       slice_offset        = threadIdx.x * {output_thread_slice_size};

          half*    const output_slice        = output + block_idx * {output_size} + slice_offset;
    const half*    const bias_slice          = bias                               + slice_offset;
          half*          shared_output_slice = shared_output                      + slice_offset;
    ...
    #pragma unroll
    for (uint32_t s = 0; s < {output_thread_slice_size} / 2; ++s)
    {{
        ((__half2*)shared_output_slice)[s] = ((__half2*)bias_slice)[s];
    }}
    if ({output_thread_slice_size} % 2 == 1)
    {{
        shared_output_slice[{output_thread_slice_size} - 1] = bias_slice[{output_thread_slice_size} - 1];
    }}
    ...
    {{
        ...
        if (feature_index != -1)
        {{
            const half* const weight_slice = weight + feature_index * {output_size} + slice_offset;
            const __half2 fv2 = __half2half2(feature_value);
            #pragma unroll
            for (uint32_t s = 0; s < {output_thread_slice_size} / 2; ++s)
            {{
                __half2 w2 = ((__half2*)weight_slice)[s];
                __half2 so2 = ((__half2*)shared_output_slice)[s];
                ((__half2*)shared_output_slice)[s] = __hfma2(w2, fv2, so2);
            }}
            if ({output_thread_slice_size} % 2 == 1)
            {{
                shared_output_slice[{output_thread_slice_size} - 1] = __hfma(weight_slice[{output_thread_slice_size} - 1], feature_value, shared_output_slice[{output_thread_slice_size} - 1]);
            }}
        }} else break;
    }}
    ...
    for (uint32_t s = 0; s < {output_thread_slice_size} / 2; ++s)
    {{
        ((__half2*)output_slice)[s] = ((__half2*)shared_output_slice)[s];
    }}
    if ({output_thread_slice_size} % 2 == 1)
    {{
        output_slice[{output_thread_slice_size} - 1] = shared_output_slice[{output_thread_slice_size} - 1];
    }}
}}
...
void feature_transformer_slice_backward(
    ...
    __shared__
          half           shared_output_grad[{output_size}];

    const uint32_t       block_idx                = blockIdx.x;
    const uint32_t       slice_offset             = threadIdx.x * {output_thread_slice_size};

    const half*    const output_grad_slice        = output_grad + block_idx * {output_size} + slice_offset;
          half*    const bias_grad_slice          = bias_grad                               + slice_offset;
          half*          shared_output_grad_slice = shared_output_grad                      + slice_offset;
    ...
    for (uint32_t s = 0; s < {output_thread_slice_size} / 2; ++s)
    {{
        ((__half2*)shared_output_grad_slice)[s] = ((__half2*)output_grad_slice)[s];
    }}
    if ({output_thread_slice_size} % 2 == 1)
    {{
        shared_output_grad_slice[{output_thread_slice_size} - 1] = output_grad_slice[{output_thread_slice_size} - 1];
    }}

    #pragma unroll
    for (uint32_t s = 0; s < {output_thread_slice_size}; ++s)
    {{
        const half sog = shared_output_grad_slice[s];
        if (__hne(sog, __float2half(0.0f)))
        {{
    ...
    {{
        ...
        if (feature_index != -1)
        {{
            half* const weight_grad_slice = weight_grad + feature_index * {output_size} + slice_offset;
            #pragma unroll
            ...
            {{
                ...
                if (__hne(sog, __float2half(0.0f)))
                {{
                    atomicAdd(&weight_grad_slice[s], __hmul(sog, feature_value));
                }}
            }}
        }} else break;
    }}
...
if __name__ == "__main__":
    ...
    def FeatureTransformerSliceFunctionEmulate(
        ...
        max_active_features = feature_indices.shape[1]
        inputs = torch.zeros(
            batch_size, num_inputs, dtype=torch.float16, device=weight.device
        )
    ...
    def test():
        ...
        torch.manual_seed(0)
        weight0 = torch.rand(
            INPUT_SIZE, STRIDE, dtype=torch.float16, requires_grad=True, device=device
        )
        bias0 = torch.rand(STRIDE, dtype=torch.float16, requires_grad=True, device=device)
        torch.manual_seed(0)
        weight1 = torch.rand(
            INPUT_SIZE, STRIDE, dtype=torch.float16, requires_grad=True, device=device
        )
        bias1 = torch.rand(STRIDE, dtype=torch.float16, requires_grad=True, device=device)
        indices0 = (torch.rand(BATCH_SIZE, MAX_ACTIVE_FEATURES, device=device) * INPUT_SIZE).to(
            dtype=torch.int32
        )
        indices1 = (torch.rand(BATCH_SIZE, MAX_ACTIVE_FEATURES, device=device) * INPUT_SIZE).to(
            dtype=torch.int32
        )
        values0 = torch.rand(BATCH_SIZE, MAX_ACTIVE_FEATURES, dtype=torch.float16, device=device)
        values1 = torch.rand(BATCH_SIZE, MAX_ACTIVE_FEATURES, dtype=torch.float16, device=device)
        ...
        output10, output11 = DoubleFeatureTransformerSliceFunction.apply(
            indices0.clone(),  # Already on GPU
            values0.clone(),   # Already on GPU
            indices1.clone(),  # Already on GPU
            values1.clone(),   # Already on GPU
            weight1,           # Already on GPU
            bias1,             # Already on GPU
        )

        print(torch.max(output00.cpu() - output10.cpu()).item())
    ...
    def bench():
        ...
        indices0 = torch.cat(
            [
                torch.sort(
                    (torch.rand(BATCH_SIZE, MAX_ACTIVE_FEATURES * 3 // 4, device=device) * INPUT_SIZE),
                    dim=1,
                )[0].to(dtype=torch.int32),
                torch.full(
                    (BATCH_SIZE, MAX_ACTIVE_FEATURES // 4), -1, dtype=torch.int32, device=device
                ),
            ],
            dim=1,
        )
        values0 = torch.rand(
            BATCH_SIZE, MAX_ACTIVE_FEATURES, dtype=torch.float16, device=device
        )
        indices1 = torch.cat(
            [
                torch.sort(
