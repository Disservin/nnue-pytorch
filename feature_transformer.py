...
void feature_transformer_slice_forward(
    ...
    {{
        ...
        {{
            ...
            for (uint32_t s = 0; s < {output_thread_slice_size}; ++s)
            {{
                shared_output_slice[s] = __hadd(shared_output_slice[s], __hmul(weight_slice[s], feature_value));
            }}
        }} else break;
    }}
...
void feature_transformer_slice_backward(
    ...
    {{
        ...
        if (!__heq(sog, __float2half(0.0f)))
        {{
...
if __name__ == '__main__':
    ...
    def test():
        ...
        STRIDE = 128
        MAX_ERROR = 1e-2  # Increased tolerance for FP16
