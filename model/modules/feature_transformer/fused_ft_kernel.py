...
def make_fused_double_ft_forward_kernel(
    max_active_indices: int, l1_size: int, *, cross_dimensions: int = 0
):
    l1_half = l1_size // 2
    num_threads = _get_num_threads_for_forward(l1_half)
    output_thread_slice_size = l1_half // num_threads

    key = (max_active_indices, l1_size, num_threads, cross_dimensions)
...
void fused_double_ft_forward(
    ...
    const uint32_t slice_offset = threadIdx.x * """
            + str(output_thread_slice_size)
            + r""";

    const float us_val = __ldg(&us[block_idx]);
    const float them_val = __ldg(&them[block_idx]);

    const int32_t* const w_idx_row = white_indices + block_idx * """
            + str(max_active_indices)
            + r""";
    const int32_t* const b_idx_row = black_indices + block_idx * """
            + str(max_active_indices)
            + r""";
    ...
    const int32_t l1_size = """
            + str(l1_size)
            + r""";
    const int32_t l1_half = """
            + str(l1_half)
            + r""";
    const int32_t cross_dimensions = """
            + str(cross_dimensions)
            + r""";
    const int32_t local_dimensions = l1_half - cross_dimensions;
    ...
    for (uint32_t s = 0; s < """
        ...
        if (cross_dimensions != 0 && i >= local_dimensions) {
            l0_out[block_idx * l1_size + i] = l0_w0 * l0_b1;
            l0_out[block_idx * l1_size + l1_half + i] = l0_b0 * l0_w1;
        } else {
...
@torch.compiler.disable(recursive=False)
def make_fused_double_ft_backward_kernel(
    max_active_indices: int,
    l1_size: int,
    tile_size: int = BACKWARD_TILE_SIZE,
...
void fused_double_ft_backward(
    ...
    __shared__ float shared_grad_bias["""
            + str(output_size)
            + r"""];
    ...
    for (int t = 0; t < tile_size; ++t) {
        ...
        for (uint32_t s = 0; s < """
            ...
            float dw0, dw1, db0, db1;
            if (cross_dimensions != 0 && i >= local_dimensions) {
                dw0 = gl0_i   * clamped_b1;
                dw1 = gl0_i_h * clamped_b0;
                db0 = gl0_i_h * clamped_w1;
                db1 = gl0_i   * clamped_w0;
            } else {
                dw0 = gl0_i   * clamped_w1;
                dw1 = gl0_i   * clamped_w0;
                db0 = gl0_i_h * clamped_b1;
                db1 = gl0_i_h * clamped_b0;
            }
            if (clamped_w0 == 0.0f || clamped_w0 == max_ft_act) dw0 = 0.0f;
            if (clamped_w1 == 0.0f || clamped_w1 == max_ft_act) dw1 = 0.0f;
            if (clamped_b0 == 0.0f || clamped_b0 == max_ft_act) db0 = 0.0f;
            if (clamped_b1 == 0.0f || clamped_b1 == max_ft_act) db1 = 0.0f;
